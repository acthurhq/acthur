...
- 2026-07-06 — main — Slice 3 (CLI + live witness). `acthur generate from-contract <file>` (bare name or path; `--node`) runs the pipeline per go:fiber node and persists via the write engine; `acthur generate model <Name> <field:type>...` emits `internal/models/<snake>.go` + test + migration (auto ULID id pk when not declared). Migration renumbering: the pipeline's fixed 0400 is remapped at the CLI — same description reuses its number, a new one takes the next free slot in 0400–0499 (golang-migrate rejects duplicate versions; caught at design time, verified by test and witness). Live witness on a fresh scaffolded project: from-contract wrote all 12 files; rerun regenerated cleanly (lock hash match); a hand-edited service.go was skipped with the pointed generated.lock warning; `generate model Pet` landed at 0401; `go build ./...` and ALL generated tests passed in the witness node. Full suite green.
...
## Acceptance Criteria
- [x] `acthur generate from-contract users.contract.yml` on a scaffolded go:fiber project → `go build ./... && go test ./...` green in the node
- [x] Every generated source file has a generated test beside it
- [x] Regeneration is idempotent; a user-edited file is skipped with a warning (generated.lock)
- [x] `acthur generate model <Name> <field:type>...` produces model + migration + test
- [x] Full suite green
