...
// a Go Gin application. It has zero knowledge of plugins or contracts.
package gin
...
// Adapter implements adapter.Adapter for Gin.
type Adapter struct{}
...
func (a *Adapter) Name() string               { return "go:gin" }
func (a *Adapter) Category() adapter.Category { return adapter.CategoryBackend }

// Detect returns true if a go.mod containing gin is found in dir.
func (a *Adapter) Detect(dir string) bool {
	...
	return contains(string(data), "github.com/gin-gonic/gin")
}
...
// EnvVars returns the environment variables a Gin app requires.
...
// Scaffold returns the minimal file set for a new go:gin service.
...
// The go:gin service node (Dockerizable capability). It is distinct from the
